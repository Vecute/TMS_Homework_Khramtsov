...
import {
  ...
  dislikePost,
  resetLikesDislikes,
} from "../redux/likesDislikesReducer";
...
export const PostCard = ({
  ...
  const likesDislikes = useSelector(
    (state: RootState) => state.likesDislikesReducer[id]
  ) || { likes: 0, dislikes: 0 }; // Получение количества лайков/дизлайков или дефолтное значение
  const likeState = useSelector(
    (state: RootState) => state.likeStatesReducer[id]
  ); // Получение состояния лайка (лайкнут, дизлайкнут или null)
  const isFavorite = useSelector(
    (state: RootState) => state.favoritesReducer[id]
  ); // Получение флага "в избранном"

  // Обработчик клика по кнопке лайка
  const handleLikeClick = (event: React.MouseEvent) => {
    event.stopPropagation(); // Остановка всплытия события
    if (likeState === undefined || likeState === "dislike") {
      // Если не лайкали или дизлайкнули
  ...
  const handleDislikeClick = (event: React.MouseEvent) => {
    event.stopPropagation(); // Остановка всплытия события
    if (likeState === undefined || likeState === "like") {
      // Если не дизлайкали или лайкнули
  ...
  const handleClick = (event: React.MouseEvent) => {
    ...
    const isButtonClick =
      ...
      target.closest(".post-card__button"); // Проверяем, кликнули ли по кнопке внутри карточки
    ...
    if (
      ...
      // Если клик был не по кнопке, не по модальному окну, не по странице пользовательских постов и в адресной строке нету /my-posts/
      navigate(`/posts/${id}`); // Переходим на страницу поста
    }

    if (!isButtonClick && !isModalClick && myPosts) {
      // Если клик был не по кнопке, не по модальному окну, но по странице пользовательских постов
    ...
    >
      ...
      <div className="post-card__bottom-menu">
        <div className="post-card__likes-menu">
          ...
          <span className="post-card__like-counter">
            {likesDislikes.likes - likesDislikes.dislikes}
          </span>
          <div
        ...
        <div className="post-card__extra-menu">
          <div
            className={`post-card__favorite post-card__button post__icons-container ${
